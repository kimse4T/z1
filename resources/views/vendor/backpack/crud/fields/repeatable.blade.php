...
@php
  ...
  {
    //get Value from table Property_title_deeds
    $field['value'] = old($field['name']) ? old($field['name']) : (isset($entry->titledeeds) ? $entry->titledeeds : (isset($field['default']) ? $field['default'] : '' ));
    // make sure the value is a JSON string (not array, if it's cast in the model)
    $field['value'] = is_array($entry->titledeeds) ? json_encode($entry->titledeeds) : $entry->titledeeds;
      ...
      @if (isset($field['fields']) && is_array($field['fields']) && count($field['fields']))
        ...
        @foreach($field['fields'] as $subfield)
          @php
            ...
            //   //if($subfield->name="title_deed_no"){
            //       dd($subfield);
            //   // }
          @endphp
